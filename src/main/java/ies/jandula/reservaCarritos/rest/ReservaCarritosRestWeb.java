...
@RequestMapping(value = "/incidenciasTic", produces =
{ "application/json" })
@RestController
@Log4j2
...
//			Para cualquier error inesperado, devolverá un 500
			...
			@RequestHeader(value = "nAlumnos", required = true) int nAlumnos)
	{
		try
		{
			...
			Optional<Reserva> optinalReserva = this.reservaRepository.encontrarReserva(email, aulaYCarritos,
			...
			Optional<Profesor> profesor = this.profesorRepository.findById(email);
