...
//	Consulta que recupera la información sobre las reservas que están asociadas a un recurso específico..
	@Query(value = "SELECT d.id, t.id, NULL, NULL, NULL, NULL " + "FROM dia_semana d, tramo_horario t "
			+ "WHERE ((d.id, t.id) NOT IN ( " + "    SELECT r.dia_semana_id, r.tramo_horario_id FROM reserva_fija r "
			+ "    UNION "
			+ "    SELECT rp.dia_semana_id, rp.tramo_horario_id FROM reserva_puntual rp WHERE rp.num_semana = :numSemana)) "
			...
			+ "WHERE r2.profesor_email = p.email AND r2.recurso_id = :recurso " + "UNION "
			+ "SELECT rp2.dia_semana_id, rp2.tramo_horario_id, rp2.n_alumnos, rp2.profesor_email, "
			+ "CONCAT(p.nombre, ' ', p.apellidos), rp2.recurso_id " + "FROM reserva_puntual rp2, profesor p "
